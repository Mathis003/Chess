...
class Pawn:
    def __init__(self, rect, tile, opponent_color, first_move):
        self.rect = rect # Rect of the piece's image
        self.tile = tile # Tile where the pieces is
        self.opponent_color = opponent_color  # -1 if white, 1 if black
    ...
    def update_possible_moves(self):
        ...
        list_possible_moves = []
        if dico_board[(self.tile[0] + self.opponent_color, self.tile[1])][2] == 0: # If the tile above the pawn is empty
            list_possible_moves.append([self.tile[0] + self.opponent_color, self.tile[1]]) # Add the tile to the possible moves

            if self.first_move: # If the pawn is on its first move and the tile above is empty
                if dico_board[(self.tile[0] + 2 * self.opponent_color, self.tile[1])][2] == 0: # If the tile above (x2) the pawn is empty
                    list_possible_moves.append([self.tile[0] + 2 * self.opponent_color, self.tile[1]]) # Add the tile to the possible moves

        for i in range(-1, 2, 2): # i = -1 or 1
            try:
                if dico_board[self.tile[0] + self.opponent_color, self.tile[1] + i][2] == self.opponent_color: # If the tile diagonally above the pawn is an opponent piece
                    list_possible_moves.append([self.tile[0] + self.opponent_color, self.tile[1] + i]) # Add the tile to the possible moves
            except:
...
class King:
    ...
    def __init__(self, rect, tile, opponent_color):
        self.rect = rect  # Rect of the piece's image
        self.tile = tile  # Tile where the pieces is
        self.opponent_color = opponent_color  # -1 if white, 1 if black
            ...
            for j in range(-1, 2): # j = -1 / 1 or 0
                try:
                    if (i, j) != (0,0): # If the tile is not the same as the king
                        if dico_board[(self.tile[0] + i, self.tile[1] + j)][2] in [0, self.opponent_color]: # If the tile is empty or an opponent piece
        ...
        for i in range(-2, 3, 4): # i = -2 or 2
            for j in range(-1, 2, 2): # j = -1 or 1
                try:
                    ...
                    pass # Deal with the out of range error$
                try:
                    if dico_board[(self.tile[0] + j, self.tile[1] + i)][2] in [0, self.opponent_color]: # If the tile is empty or an opponent piece
    ...
    def update_possible_moves(self):
        ...
        try:
            for i in range(1, self.tile[0] + 1): # Move above the rook piece
                ...
                if dico_board[(self.tile[0] - i, self.tile[1])][2] in [self.opponent_color, 2 * self.opponent_color]: # If the tile is occured by an opponent piece
                    list_possible_moves.append([self.tile[0] - i, self.tile[1]]) # Add the tile to the possible moves
                    break # Stop the loop
                elif dico_board[(self.tile[0] - i, self.tile[1])][2] == - self.opponent_color: # If the tile is an ally piece
                    break # Stop the loop
        except:
            pass # Deal with the out of range error
        ...
        try:
            for i in range(1, self.tile[1] + 1): # Move to the left of the rook piece
                ...
                if dico_board[(self.tile[0], self.tile[1] - i)][2] in [self.opponent_color, 2 * self.opponent_color]:  # If the tile is empty
                    list_possible_moves.append([self.tile[0], self.tile[1] - i])  # Add the tile to the possible moves
                    break # Stop the loop
                elif dico_board[(self.tile[0], self.tile[1] - i)][2] == - self.opponent_color: # If the tile is an ally piece
                    break # Stop the loop
        except:
            pass # Deal with the out of range error
        try:
        ...
        for i in range(1, ROW):
            try:
                if enter_left_up:
                    ...
                    elif dico_board[(self.tile[0] - i, self.tile[1] - i)][2] in [self.opponent_color, 2 * self.opponent_color]:  # If the tile is empty or an opponent piece
                        list_possible_moves.append([self.tile[0] - i, self.tile[1] - i])  # Add the tile to the possible moves
                        enter_left_up = False
                    elif dico_board[(self.tile[0] - i, self.tile[1] - i)][2] == - self.opponent_color:  # If the tile is an ally piece
                        enter_left_up = False
            except:
    ...
    def update_possible_moves(self):
        ...
        for i in range(1, ROW):
            try:
                if enter_left_up:
                    ...
                    elif dico_board[(self.tile[0] - i, self.tile[1] - i)][2] in [self.opponent_color,
                                                                                 2 * self.opponent_color]:  # If the tile is empty or an opponent piece
                        list_possible_moves.append(
                            [self.tile[0] - i, self.tile[1] - i])  # Add the tile to the possible moves
                        enter_left_up = False
                    elif dico_board[(self.tile[0] - i, self.tile[1] - i)][
                        2] == - self.opponent_color:  # If the tile is an ally piece
                        enter_left_up = False
            except:
                enter_left_up = False  # Deal with the out of range error
            try:
        ...
        try:
            for i in range(1, self.tile[0] + 1):  # Move above the rook piece
                if dico_board[(self.tile[0] - i, self.tile[1])][2] == 0:  # If the tile is empty
                    list_possible_moves.append([self.tile[0] - i, self.tile[1]])  # Add the tile to the possible moves
                if dico_board[(self.tile[0] - i, self.tile[1])][2] in [self.opponent_color,
                                                                       2 * self.opponent_color]:  # If the tile is occured by an opponent piece
                    list_possible_moves.append([self.tile[0] - i, self.tile[1]])  # Add the tile to the possible moves
                    break  # Stop the loop
        ...
        try:
            for i in range(1, self.tile[1] + 1):  # Move to the left of the rook piece
                if dico_board[(self.tile[0], self.tile[1] - i)][2] == 0:  # If the tile is empty
                    list_possible_moves.append([self.tile[0], self.tile[1] - i])  # Add the tile to the possible moves
                if dico_board[(self.tile[0], self.tile[1] - i)][2] in [self.opponent_color,
                                                                       2 * self.opponent_color]:  # If the tile is empty
                    list_possible_moves.append([self.tile[0], self.tile[1] - i])  # Add the tile to the possible moves
                    break  # Stop the loop
